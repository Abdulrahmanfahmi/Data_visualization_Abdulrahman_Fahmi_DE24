...
def perform_calculation(state):
    ...
    state.quotient = int(state.number1) / int(state.number2) if int(state.number2) != 0 else "Cannot divide by zero"
    ...
    with tgb.part(class_name="container card"):
        with tgb.layout(columns="1 1 1"):
            ...
            with tgb.part():    
                ...
                tgb.text("{number1} - {number2} = {difference}")
                tgb.text("{number1} * {number2} = {product}")
                tgb.text("{number1} / {number2} = {quotient}")
                
            
            with tgb.part() as column_data:
                tgb.table("{df.head(100)}")
